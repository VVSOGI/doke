...
export function useFormattedCommand() {
    ...
    Object.entries(headers).forEach(([key, value]) => {
      if (key === 'credentials') {
        const credentials = (value as HeaderCredential[]).map((credential) => {
          return `-H "${credential.key}: ${credential.type === 'custom' ? '' : credential.type + ' '}${credential.value}" \\\n`
        })
  ...
  useEffect(() => {
    ...
    const totalCommand =
      startCommand + formattedParams + formattedQuerys + formattedHeaders + (formattedBodies && `-d '${formattedBodies}'`)
