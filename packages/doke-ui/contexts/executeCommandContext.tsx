...
import { processHeaders, processQueryParameters, processRequestBody, processUrlParameters } from '@/lib/utils/processRequestJSON'
...
export function ExecuteCommandProvider({ children, projectData, controllerData, selected, setSelected }: ExecuteCommandProviderProps) {
  ...
  const getFormattedUrl = (selected: Endpoint) => {
    return `${projectData.serverUrl}${controllerData.basePath ? '/' + controllerData.basePath : ''}` + selected.path.split('/:')[0]
  }
